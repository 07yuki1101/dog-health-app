  ...
  return (
    <div className="max-w-lg mx-auto pb-28">
      <div className="px-4 pt-6 pb-4">
        <div className="flex items-center gap-1.5 bg-violet-100 text-violet-600 px-3 py-1.5 rounded-full inline-flex">
          <span className="text-sm">👨‍👩‍👧</span>
          <span className="text-sm font-black">家族の管理</span>
        </div>
      </div>
        ...
        <div className="bg-white rounded-3xl p-5 shadow-sm">
          ...
          {notifStatus === "granted" && (
            <div className="flex items-center gap-2 text-green-600">
              <span className="text-lg">🔔</span><span className="text-sm font-bold">通知が有効です</span>
            </div>
          )}
          {notifStatus === "denied" && (
            <div>
              <div className="flex items-center gap-2 text-red-500 mb-1"><span className="text-lg">🔕</span><span className="text-sm font-bold">通知がブロックされています</span></div>
              <p className="text-xs text-gray-400">ブラウザの設定から通知を許可してください。</p>
            </div>
          )}
          {notifStatus === "unsupported" && <p className="text-sm text-gray-400">このブラウザはプッシュ通知に対応していません。</p>}
          {notifStatus === "default" && (
